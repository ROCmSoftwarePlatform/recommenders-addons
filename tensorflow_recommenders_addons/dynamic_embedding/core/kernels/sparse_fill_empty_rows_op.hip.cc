...
void SparseFillEmptyRowsGpuImpl(OpKernelContext* context,
  ...
  hipMemcpyAsync(&dense_row_number, input_shape, sizeof(int64),
                  hipMemcpyDeviceToHost, OpStream);
  hipStreamSynchronize(OpStream);
  ...
  hipMemset(row_nnz_count.flat<int>().data(), 0,
             sizeof(int) * dense_row_number);
  hipMemset(input_row_offset.flat<int64>().data(), 0, sizeof(int64));
  hipMemset(output_row_offset.flat<int64>().data(), 0, sizeof(int64));
  ...
  int64 output_nnz;
  hipMemcpyAsync(&output_nnz,
                  output_row_offset.flat<int64>().data() + dense_row_number,
                  sizeof(int64), hipMemcpyDeviceToHost, OpStream);
  hipStreamSynchronize(OpStream);
  ...
  if (context->output_required(2)) {
    ...
    hipMemset(empty_row_indicator, false, sizeof(bool) * dense_row_number);
  }
