...
class CuckooHashTableOfTensorsGpu final : public LookupInterface {
 public:
  ...
  size_t size() const override {
    ...
    CUDA_CHECK(hipStreamSynchronize(_stream));
    CUDA_CHECK(hipStreamDestroy(_stream));
    return retv;
  }
  ...
  Status Find(OpKernelContext* ctx, const Tensor& d_keys, Tensor* value,
    ...
    if (len > 0) {
      ...
      CUDA_CHECK(hipStreamCreate(&_stream));
      CUDA_CHECK(hipMallocManaged((void**)&d_status, sizeof(bool) * len));
      {
        tf_shared_lock l(mu_);
      ...
      CUDA_CHECK(hipFree(d_default_value));
      CUDA_CHECK(hipStreamDestroy(_stream));
    }
    return Status::OK();
  }
  ...
  void RehashIfNeeded(hipStream_t stream) {
    K* d_keys;
    ...
    if (new_max_size != max_size_) {  // rehash manually.
      ...
      CUDA_CHECK(hipMallocManaged((void**)&d_dump_counter, sizeof(size_t)));
      CUDA_CHECK(hipMallocManaged((void**)&d_keys, sizeof(K) * capacity));
      CUDA_CHECK(hipMallocManaged((void**)&d_values,
      ...
      CUDA_CHECK(hipStreamSynchronize(stream));
      CUDA_CHECK(hipMemcpy((size_t*)&h_dump_counter, (size_t*)d_dump_counter,
  ...
  Status Remove(OpKernelContext* ctx, const Tensor& keys) override {
    ...
    CUDA_CHECK(hipStreamCreate(&_stream));
    if (len > 0) {
      CUDA_CHECK(hipMallocManaged((void**)&d_keys, sizeof(K) * len));
      CUDA_CHECK(hipMemcpy((void*)d_keys, (void*)keys.tensor_data().data(),
                            sizeof(K) * len, hipMemcpyDefault));
      {
        mutex_lock l(mu_);
  ...
  Status ImportValues(OpKernelContext* ctx, const Tensor& keys,
    ...
    if (len > 0) {
      ...
      CUDA_CHECK(hipMallocManaged((void**)&d_keys, sizeof(K) * len));
      CUDA_CHECK(
          hipMallocManaged((void**)&d_values, sizeof(V) * runtime_dim_ * len));
      CUDA_CHECK(hipMemcpy((void*)d_keys, (void*)keys.tensor_data().data(),
                            sizeof(K) * len, hipMemcpyDefault));
      CUDA_CHECK(hipMemcpy((void*)d_values, (void*)values.tensor_data().data(),
                            sizeof(V) * runtime_dim_ * len, hipMemcpyDefault));
      {
        mutex_lock l(mu_);
        table_->clear(_stream);
...
class HashTableSizeGpuOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    CUDA_CHECK(hipMemcpy((void*)out->tensor_data().data(), (void*)&size,
                          sizeof(size_t), hipMemcpyDefault));
  }
};
